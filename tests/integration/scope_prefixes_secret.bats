...
#   tacctl scopes prefixes <scope> {list|add|remove|clear}
#   tacctl scopes secret   <scope> {show|set|generate}
...
@test "scopes prefixes list: shows the current prefix list" {
    run "$TACCTL_BIN_SCRIPT" scopes prefixes lab list
    assert_success
    assert_output --partial "192.168.0.0/16"
}

@test "scopes prefixes list: errors on unknown scope" {
    run "$TACCTL_BIN_SCRIPT" scopes prefixes nosuchscope list
    assert_failure
...
@test "scopes prefixes add: appends a new CIDR" {
    run "$TACCTL_BIN_SCRIPT" scopes prefixes lab add 172.16.0.0/12
    assert_success
    run "$TACCTL_BIN_SCRIPT" scopes prefixes lab list
    assert_output --partial "192.168.0.0/16"
...
@test "scopes prefixes add: canonicalizes host bits on input" {
    run "$TACCTL_BIN_SCRIPT" scopes prefixes lab add 172.16.5.5/12
    assert_success
    run "$TACCTL_BIN_SCRIPT" scopes prefixes lab list
...
@test "scopes prefixes add: accepts comma-separated list" {
    run "$TACCTL_BIN_SCRIPT" scopes prefixes lab add "172.16.0.0/12,100.64.0.0/10"
    assert_success
    run "$TACCTL_BIN_SCRIPT" scopes prefixes lab list
    assert_output --partial "172.16.0.0/12"
...
@test "scopes prefixes remove: drops the CIDR from the list" {
    "$TACCTL_BIN_SCRIPT" scopes prefixes lab add 172.16.0.0/12 > /dev/null
    run "$TACCTL_BIN_SCRIPT" scopes prefixes lab remove 192.168.0.0/16
    assert_success
...
@test "scopes prefixes remove: warns when CIDR isn't present" {
    run "$TACCTL_BIN_SCRIPT" scopes prefixes lab remove 100.64.0.0/10
...
@test "scopes prefixes clear: wipes all prefixes from an unreferenced scope" {
    run bash -c 'echo y | "'"$TACCTL_BIN_SCRIPT"'" scopes prefixes lab clear'
    assert_success
...
@test "scopes prefixes clear: --force clears with users present (leaves orphan refs)" {
    "$TACCTL_BIN_SCRIPT" user add alice superuser \
        --hash "$TEST_HASH" --scopes lab > /dev/null
    run bash -c 'echo y | "'"$TACCTL_BIN_SCRIPT"'" scopes prefixes lab clear --force'
    assert_success
...
@test "scopes secret show: prints value and length (healthy)" {
    run "$TACCTL_BIN_SCRIPT" scopes secret prod show
    assert_success
...
@test "scopes secret show: flags REPLACE placeholder secrets" {
    ...
    "$TACCTL_BIN_SCRIPT" scopes add placeholder \
        --prefixes 198.51.100.0/24 --secret "REPLACE_WITH_REAL_SECRET" > /dev/null
    run "$TACCTL_BIN_SCRIPT" scopes secret placeholder show
    assert_success
...
@test "scopes secret set: persists the new secret" {
    run "$TACCTL_BIN_SCRIPT" scopes secret prod set "NEW-prod-secret-0123456789"
    assert_success
    run "$TACCTL_BIN_SCRIPT" scopes secret prod show
...
@test "scopes secret set: rejects value below SECRET_MIN_LENGTH" {
    run "$TACCTL_BIN_SCRIPT" scopes secret prod set "short"
    assert_failure
    assert_output --partial "minimum is"
}

@test "scopes secret set: rejects single-character-class secrets (low entropy)" {
    run "$TACCTL_BIN_SCRIPT" scopes secret prod set "aaaaaaaaaaaaaaaaaaaa"
    assert_failure
    assert_output --partial "single-character-class"
    run "$TACCTL_BIN_SCRIPT" scopes secret prod set "12345678901234567890"
    assert_failure
}
...
@test "scopes secret generate: uses openssl and persists the value" {
    run "$TACCTL_BIN_SCRIPT" scopes secret prod generate
    assert_success
...
@test "scopes secret show: errors on unknown scope" {
    run "$TACCTL_BIN_SCRIPT" scopes secret nosuchscope show
    assert_failure
    assert_output --partial "does not exist"
}
